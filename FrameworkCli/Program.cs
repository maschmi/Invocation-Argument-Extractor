...
using Inw.ArgumentExtractor.MSBuildLocator;
    ...
    public class Program
    {
        ...
        private static async Task RunAnalysis(ProgramOptions options)
        {
            ...
            using (var solLoader = new SolutionLoader(new WorkspaceCreator(logger: _logger), _logger))
            {
                ...
                var declaredSymbols = await extractor.FindSymbols(solution, options.FullTypeName, options.MethodName,
                    options.MethodArguments);
                if(declaredSymbols.Count() == 0)
                    _logger.Info(
                        $"Did not find anything for {options.FullTypeName}.{options.MethodName}({string.Join(", ", options.MethodArguments)})");
